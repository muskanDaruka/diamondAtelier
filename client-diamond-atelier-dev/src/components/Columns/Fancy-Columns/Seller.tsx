...
export const sellerColumn: ColumnDef<Fields, any>[] = [
  ...
  },
  {
    ...
    cell: (info) => <span>{`${info.getValue() ? ApproxPiece(info.getValue()) : "-"}`}</span>,
  },
  {
    id: 'approx_weight',
  ...
  },
  {
    ...
    cell: ({ row }: any) => <span>${(row.original.WGT || 300) / Number(ApproxPiece(row?.original?.SIZE_N)) * row.original.no_of_piece || 0}</span>,
  },

...
export const superAdminColumn: ColumnDef<Fields, any>[] = [
  ...
  },
  {
    ...
    header: "Color",
    cell: (info) => (
      <span className="text-nowrap">{`${info.getValue() ? `${info.getValue()}` : "-"
        }`}</span>
    ),
  },
  {
  ...
  },
  {
    ...
    size: 90,
    cell: (info) => (
      <span>{`${info.getValue() ? `${parseFloat(info.getValue()).toFixed(2)}` : "-"
        }`}</span>
    ),
  },
  {
  ...
  {
    id: "memo_dtl",
    accessorKey: "memo_dtl",
    header: "Memo Details",
    size: 80,
    cell: ({ row }: any) => <MemoDetails rows={row.original} />,
  },
];

...
import { getCsMeleeApi } from "@/redux/color/cs-melee/getCsMeleeReducer";
import CompareStock from "@/components/ResultCs-Melee/CompareStock";
import DownloadStock from "@/components/ResultCs-Melee/DownloadStock";
import SendEmail from "@/components/ResultCs-Melee/SendEmail";
import ShowCalculation from "@/components/ResultCs-Melee/ShowCalculation";
import SendWhatsapp from "@/components/ResultCs-Melee/SendWhatsapp";
import AddToCart from "@/components/ResultCs-Melee/AddToCart";
import HoldStone from "@/components/ResultCs-Melee/HoldStone";
import MakeToOrder from "@/components/common/MakeToOrder";

const PrintSelectedRow = dynamic(() => import('@/components/ResultCs-Melee/PrintStock'), { ssr: false });
...
function Page() {
  const { data, filters, isLoading } = useAppSelector(
    (store: RootState) => store?.getCsMeleeReducer
  );
  ...
  useEffect(() => {
    ...
    if (!data?.data?.Table?.length && hasLoad !== false) {
      dispatch(getCsMeleeApi({ ...filters, PAGENO: 1 })).then((res) => res).then((res: any) => {
    ...
    <div className="flex font-serif h-screen flex-col">
      <section className=" flex flex-1 bg-white">
        <div className="relative flex h-full flex-1 flex-col overflow-y-auto">
          <main className="flex-1 py-5 lg:px-4 px-0.5">
            <div>
            <div className="grid grid-cols-1 lg:grid-cols-3 w-[98%] mx-auto gap-2 p-1">
                ...
                <div className="text-center flex justify-center gap-5 items-center font-serif">
                  ...
                  <button
                    ...
                    onClick={() => replace("/cs-meleeResult?")}
                  >
                    New Search
                  </button>
                </div>

...
export const superAdminColumn: ColumnDef<Fields, any>[] = [
  ...
  },
  {
    ...
    header: "P/Ct",
    size: 100,
    cell: (info) => <span>{`${info.getValue() ? `$${parseFloat(info.getValue()).toFixed()}` : "NA"}`}</span>,
  },
  {
    id: "NET_VALUE",
    ...
    header: "Total Value",
    size: 100,
    cell: (info) => <span>{`${info.getValue() ? `$${parseFloat(info.getValue()).toFixed()}` : "-"}`}</span>,
  },
  {
    id: "PRICE_RATE",
  ...
  },
  {
    ...
    size: 120,
    cell: (info) => <span className="text-nowrap">{`${info.getValue() ? `${moment(info.getValue()).format('MM-DD-YYYY')}` : "-"}`}</span>,
  },
  {
    id: "SUPPLIER_NAME",

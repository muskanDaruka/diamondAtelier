...
export const sellerColumn: ColumnDef<Fields, any>[] = [
  {
    id: "checkbox",
    size: 50,
    cell: ({ row }) => (
        ...
        case "B":
          ...
          content = (
            <div className="grid grid-cols-4 w-full" onClick={handleClick}>
              ...
              <div className="col-span-3">
                <span className="text-black text-nowrap">{label}</span>
              </div>
            </div>
          );
          break;
  ...
  //   header: "SIZE IN (CENT)",
  //   size:100,
  //   cell: (info) => <span>{SizeInCent(info.getValue())}</span>,
  // },
  ...
  },
  {
    ...
    header: "Measurement",
    size: 100,
    cell: (info) => <span className="text-nowrap">{`${info.getValue() ? `${info.getValue().split("*").join("-")}` : "-"}`}</span>,
  },
  {
    id: "SALERATE",
